...
#Create daily_order_df() digunakan untk menyiapkan daily_orders_df
...
#Create sum_order_df() bertanggung jawab untuk menyiapkan sum_orders_items_df
def create_sum_order_items_df(df):
    sum_order_items_df = df.groupby("product_name").quantity_x.sum().sort_values(ascending=False).reset_index()
    return sum_order_items_df

#create_bygendeder_df() digunakaan untuk menyiapkan bygender_df
def create_bygender_df(df):
    ...
    return bygender_df
#create_bygender_df() merupakan helper function yang digunakan untuk menyiapkan byage_df()
...
#mengurutkan DataFrame berdasarkan order_date serta memastikan kedua kolom tsb bertipe datetime
...
#membuat filter denan widget date input serta menambahkan logo Perusahaan
...
#Header
st.header('Uhuy Collection Dashboard :sparkles:')
...
#dan paling sedikit terjual melalui sebuah visualisasi data.
...
st.caption('Copyright (c) Uhuy 2024')